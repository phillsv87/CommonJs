...
const renderers:{[key:string]:IconRenderer}={}

export interface RnIconProps extends TextProps
{
    icon:string;
    set?:'at'|'en'|'ev'|'fa'|'fp'|'fps'|'fpl'|'fpb'|'f4'|'ft'|'fo'|'fd'|'io'|'mc'|'md'|'ot'|'sl'|'zo';
    color?: string;
    size?: number;
}
...
export default function RnIcon({
    ...
    switch(set){
        ...
        // case 'fps':
        //     return <Fp {...props} name={icon} size={size} color={color} solid />

        // case 'fpl':
        //     return <Fp {...props} name={icon} size={size} color={color} light />

        // case 'fpb':
        //     return <Fp {...props} name={icon} size={size} color={color} brand />
